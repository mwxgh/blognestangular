...
export class BaseService {
  ...
  async get(options?: FindManyOptions<any>): Promise<any[]> {
   ...
   * @param values
   */
  async firstOrCreate(options: FindOneOptions, values): Promise<any> {
    let item;
   ...
   * @param data
   */
  async update(id: number | string, data): Promise<any> {
    const manager = getManager();
    const item = await this.repository.findOne(id);
   ...
   * @return entity
   */
  async updateOrCreate(attributes, values): Promise<any> {
    let item;
