...
export interface IMailable {
  ...
  subject(subject: string, customStyle?: string, display?: boolean): IMailable;
...
export class Mailable implements IMailable {
  ...
  private _customDisplaySubject: any;
  private _attachments: any[];
  private _ccs: any[];
  private _bccs: any[];
  private _from_address: string;
  private _subject: string;
  private _mergeFields: any[];
  constructor() {
   ...
   * @return self
   */
  subject(content = '', customStyle?: string, display = true): IMailable {
    if (customStyle) {
      this._customSubjectStyle = customStyle;
    }
    this._customDisplaySubject = display;
  ...
  getParams(): IMailableNodeMailerParams {
    return {
      ...
      html: this.dynamicContent(this.buildContent()),
    };
  }
  ...
  dynamicContent(html) {
    if (this._mergeFields) {
      for (const field of this._mergeFields) {
        html = this.replaceAll(html, field.name, field.value);
      }
    }
    return html;
  }

  replaceAll(html, find, replace) {
    let _replace_temp = '';
    while (_replace_temp != html) {
      _replace_temp = html;
